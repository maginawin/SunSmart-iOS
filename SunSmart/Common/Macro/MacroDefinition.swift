...
/// 窗口边距
var kWindowSafeAreaInsets: UIEdgeInsets {
    guard let window = UIApplication.shared.connectedScenes
            .compactMap({ $0 as? UIWindowScene })
            .first?.windows.first else {
        return .zero // 返回默认值，避免崩溃
    }
    return window.safeAreaInsets
}

/// 顶部导航栏边距
let kSafeAreaTopHeight = kWindowSafeAreaInsets.top

//let kSafeAreaTopHeight = StatusBarManager.statusBarFrame.size.height
/** 底部边界值  iPhone X类型使用*/
let kSafeAreaBottomHeight = kWindowSafeAreaInsets.bottom
...
func SCRXFrom(_ x : CGFloat) -> CGFloat {
    if isIPad {
        return x * SCREEN_WIDTH / iPadStandardSize.width
    }
    return x * SCREEN_WIDTH / iPhoneStandardSize.width
}
...
func SCRYFrom(_ y : CGFloat) -> CGFloat {
    ...
    return y * SCREEN_HEIGHT / iPhoneStandardSize.height
...
func SCRYFit(_ y : CGFloat) -> CGFloat {
    let standard: CGFloat = isIPad ? iPadStandardSize.height : iPhoneStandardSize.height
    return SCREEN_HEIGHT / standard * y
}
