...
//        ctx.strokePath()
        // 先绘制阴影（在边框下方）
        ctx.saveGState() // 保存当前上下文状态
        ctx.setShadow(offset: CGSize(width: 0, height: 2), blur: 4, color: UIColor.black.withAlphaComponent(0.3).cgColor)
        ctx.addPath(thumbPath.cgPath)
        ctx.fillPath()
        ctx.restoreGState() // 恢复上下文（避免阴影影响边框）
...
public class RangeSlider: UIControl {
    ...
    @IBInspectable public var lowerValue: Double {
        get {
            ...
            // 保证 upperValue - lowerValue >= minimumRange
            newValue = min(newValue, _upperValue - minimumRange)
    ...
    @IBInspectable public var upperValue: Double {
        ...
        set {
            ...
            // 保证 upperValue - lowerValue >= minimumRange
            newValue = max(newValue, _lowerValue + minimumRange)
    ...
    /// 高值与低值之间最小差值
    public var minimumRange: Double = 0 {
        didSet {
            if minimumRange < 0 {
                minimumRange = 0
            }
            ...
            // 保证总范围支持这个差值
            if maximumValue - minimumValue < minimumRange {
    ...
    /// 是否维持当前滑动的滑块高亮
    public var keepThumbHighlighted: Bool = false {
        didSet {
            if keepThumbHighlighted {
                lowerThumbLayer.strokeColor = Bar_Color
                upperThumbLayer.strokeColor = Bar_Color
            }else {
                lowerThumbLayer.strokeColor = thumbTintColor
                upperThumbLayer.strokeColor = thumbTintColor
            }
        }
    }
    ...
    private func enforceValueBounds() {
        // 保证 upperValue ≥ lowerValue + minimumRange
        if _upperValue - _lowerValue < minimumRange {
            if _lowerValue + minimumRange <= maximumValue {
                _upperValue = _lowerValue + minimumRange
            } else {
                _lowerValue = _upperValue - minimumRange
            }
        }
        
        // 保证值都在 [min, max] 范围内
        _lowerValue = max(_lowerValue, minimumValue)
        _upperValue = min(_upperValue, maximumValue)
    }
