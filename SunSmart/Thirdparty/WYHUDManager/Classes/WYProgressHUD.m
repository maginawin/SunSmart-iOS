...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"WYProgressHUD needs to be accessed on the main thread.");

CGFloat const MBProgressMaxOffset = 1000000.f;

static const CGFloat MBDefaultPadding = 13.f;
static const CGFloat MBDefaultLabelFontSize = 16.f;
static const CGFloat MBDefaultDetailsLabelFontSize = 12.f;


@interface WYProgressHUD ()
...
@interface WYProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    WYProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    WYProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (WYProgressHUD *)HUDForView:(UIView *)view {
...
- (void)commonInit {
    ...
    _animationType = WYProgressHUDAnimationFade;
    _mode = WYProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _padding = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(WYProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == WYProgressHUDAnimationZoom) {
        type = animatingIn ? WYProgressHUDAnimationZoomIn : WYProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == WYProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == WYProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == WYProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == WYProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    WYProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<WYProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    WYBackgroundView *backgroundView = [[WYBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = WYProgressHUDBackgroundStyleSolidColor;
    ...
    WYBackgroundView *bezelView = [WYBackgroundView new];
    ...
    UIButton *button = [WYProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[WYRoundProgressView class]];

    WYProgressHUDMode mode = self.mode;
    if (mode == WYProgressHUDModeIndeterminate) {
    ...
    else if (mode == WYProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[WYBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == WYProgressHUDModeDeterminate || mode == WYProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[WYRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == WYProgressHUDModeAnnularDeterminate) {
            [(WYRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == WYProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == WYProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[WYProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[WYProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[WYRoundProgressView class]]) {
        WYRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [WYRoundProgressView appearanceWhenContainedIn:[WYProgressHUD class], nil];
#else
        appearance = [WYRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[WYProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((WYRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((WYRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[WYBarProgressView class]]) {
        WYBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [WYBarProgressView appearanceWhenContainedIn:[WYProgressHUD class], nil];
#else
        appearance = [WYRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[WYProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((WYBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((WYBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    WYBackgroundView *bezelView = self.bezelView;
...
- (void)updateConstraints {
    ...
    NSDictionary *paddigns = @{@"margin": @(padding)};
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
        ...
        [bezelConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(>=margin)-[view]-(>=margin)-|" options:0 metrics:paddigns views:NSDictionaryOfVariableBindings(view)]];
        // Element spacing
        if (idx == 0) {
            // First, ensure spacing to bezel edge
            
            [bezelConstraints addObject:[NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:bezel attribute:NSLayoutAttributeTop multiplier:1.f constant:padding - margin]];
        } else if (idx == subviews.count - 1) {
            // Last, ensure spacing to bezel edge
            [bezelConstraints addObject:[NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:bezel attribute:NSLayoutAttributeBottom multiplier:1.f constant:-(padding - margin)]];
        }
        if (idx > 0) {
            // Has previous
...
- (void)setMode:(WYProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == WYProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation WYRoundProgressView
...
@implementation WYBarProgressView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = WYProgressHUDBackgroundStyleBlur;
        if (@available(iOS 13.0, *)) {
            #if TARGET_OS_TV
            _blurEffectStyle = UIBlurEffectStyleRegular;
            #else
            _blurEffectStyle = UIBlurEffectStyleSystemThickMaterialDark;
            #endif
...
- (void)setStyle:(WYProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    WYProgressHUDBackgroundStyle style = self.style;
    if (style == WYProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == WYProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation WYProgressHUDRoundedButton
