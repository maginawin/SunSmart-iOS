  //
//  PJDeviceGroupSelectionViewController.swift
//  SunSmart
//
...
final class PJDeviceGroupSelectionViewController: UIViewController {

    private let context: PJDeviceGroupSelectionContext
    ...
    init(
        context: PJDeviceGroupSelectionContext,
        selectionHandler: @escaping ([UInt16]) -> Void
    ) {
        self.context = context
        self.selectedGroupAddresses = context.selectedGroupAddresses.sorted()
    ...
    private var groups: [Group] {
        context.groups.sorted(by: { $0.address.address < $1.address.address })
    }

    private var selectableGroups: [Group] {
        groups.filter {
            !context.disabledGroupAddresses.contains($0.address.address) ||
            selectedGroupAddresses.contains($0.address.address)
        }
    }
...
extension PJDeviceGroupSelectionViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let isDisabled = context.disabledGroupAddresses.contains(address) && !isSelected
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if context.disabledGroupAddresses.contains(address) && !selectedGroupAddresses.contains(address) {
            XWHUDManager.showTipHUD(context.disabledSelectionTip, isLineFeed: true)
            return
        }
