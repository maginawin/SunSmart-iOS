//
//  DeviceOthersCollectionViewCell.swift
//  SunSmart
//
...
class DeviceOthersCollectionViewCell: UICollectionViewCell {
    ...
    var deleteActionCallback: ((DeviceDongleData)->Void)?
        ...
        didSet {
            ...
            if let node = dongle.bindNode {
                if node.isKeybindComplete {
                    if !node.state {
                        iconImageView.image = UIImage(named: "device_dongle_offline")
                    }else if dongle.needSyncData {
                        iconImageView.image = UIImage(named: "sync_failed_big")
                    }
                }else {
...
//            make.top.equalTo(SCRXFrom(-5))
    ...
    private func updateUI() {
        ...
        if self.dongle.bindNode != nil { // 已设备代理设备
            self.deleteDashedBorder()
        }else { // 未设置代理设备
            self.addDashedBorder()
        }
        backgroundColor = self.dongle.collectionEnable ? .white : RGB(226, 226, 226)
    }
    
    @objc private func deleteBtnClick() {
        deleteActionCallback?(dongle)
    }
