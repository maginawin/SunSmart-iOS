        ...
        vc.settingsCompletionCallback = {[weak self] result in
            ...
            result.forEach { item in
                ...
                item.failedNodes.forEach({ node in
                        guard let type = item.parameter else {
                            return
                        }
                        if var data = self.settingFailedDatas[node.primaryUnicastAddress] {
...
extension DeviceParameterDevicesViewController: DeviceParameterPromptViewDelegate {
    ...
    func promptViewFilterAction(_ view: DeviceParameterPromptView) {
        ...
        var changeControlPageSelectIndex: Int?
        switch self.filterChangeControlPage {
        case .selected(let value, _):
            if let page = value as? NodeChangeControlPage {
                changeControlPageSelectIndex = changeControlPageContents.firstIndex(where: { $0.value == page })
            }
        default:
            break
        }

        let absoluteCctRangeContents: [(name: String, value: ClosedRange<UInt16>)] = absoluteCctRanges.map({ range in
            ("\(range.lowerBound)K~\(range.upperBound)K", range)
        })
        var absoluteCctRangeSelectIndex: Int?
        switch self.filterAbsoluteCctRange {
        case .selected(let value, _):
            if let range = value as? ClosedRange<UInt16> {
                absoluteCctRangeSelectIndex = absoluteCctRangeContents.firstIndex(where: { $0.value == range })
            }
        default:
            break
        }
...
extension Node {
    ...
    var tempChangeControlPage: NodeChangeControlPage {
        get {
            objc_getAssociatedObject(self, &Node.tempChangeControlPageKey) as? NodeChangeControlPage ?? effectiveChangeControlPage
        }set {
            objc_setAssociatedObject(self, &Node.tempChangeControlPageKey, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }
    ...
    var tempAbsoluteCctRange: ClosedRange<UInt16> {
        get {
            objc_getAssociatedObject(self, &Node.tempAbsoluteCctRangeKey) as? ClosedRange<UInt16> ?? effectiveCctRange
        }set {
            objc_setAssociatedObject(self, &Node.tempAbsoluteCctRangeKey, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }
}
