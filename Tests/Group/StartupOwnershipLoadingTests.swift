...
final class CloudSynchronizationManager {
    ...
    var uploads = 0
    func getSpaceCurrentSyncState(_ space: SpaceData) -> Int? { nil }
...
enum StartupOwnershipLoadingTests {
    ...
    static func main() throws {
        try testProjection()
        testScheduling()
        print("PASS: real SQLite identity projection, no model decoding, scope isolation, coalescing, main queue handoff and stale recovery")
    }
