# Lumineux Device 与 Group Control Big 图标矢量重绘设计

## 目标

以当前已确认正确的 `device_control_off/on` 与 `group_off/on` 40pt Retina 图标为视觉基准，生成对应的 56pt `_big` 图标，替换 Lumineux 现有错误资源。生成过程不得对 80/120px PNG 做插值放大。
...
- `device_control_off_big` 与 `group_off_big`：透明背景、Lumineux 蓝色外圆描边和蓝色电源符号。
- `device_control_on_big` 与 `group_on_big`：Lumineux 蓝色实心圆和白色电源符号。
...
只修改以下四个 imageset，不修改生产 Swift、其他 Device/Group 资源或用户现有改动：
...
- `Lumineux/Assets-Lumineux.xcassets/Group/group_off_big.imageset`
- `Lumineux/Assets-Lumineux.xcassets/Group/group_on_big.imageset`
...
- 分别运行 Device 与 Group 的 Lumineux 资源合同测试，以及覆盖 iPad `_big` 资源的 focused UIKit 测试，并人工检查实际布局截图。
